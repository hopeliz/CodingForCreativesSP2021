...
We have a LOT of repetition here - the color, the location of the squares - even the size is based off the size of the first square. Let's turn those things into reuable variables so we can update the whole thing easily.
...
Then, replace the values in the code to the variables.
